...
import OAuth2TokenGrantTypes from '../model/OAuth2TokenGrantTypes';
import OAuthToken from '../model/OAuthToken';
import OAuthTokenIntrospection from '../model/OAuthTokenIntrospection';
import OpenIDUserInfo from '../model/OpenIDUserInfo';
...
export default class AuthApi {
    ...
    */
    ...
    /**
     * Get userinfo (OpenID Endpoint).
     * Returns information about the authenticated user.  The access token obtained must be sent as a bearer token in the `Authorization` header.  This endpoint implements the [OpenID UserInfo Endpoint](https://openid.net/specs/openid-connect-core-1_0.html#UserInfo).
    ...
    getUserinfoWithHttpInfo() {
      ...
      let contentTypes = [];
      let accepts = ['application/json'];
      let returnType = OpenIDUserInfo;
      return this.apiClient.callApi(
        '/auth/oauth/userinfo', 'GET',
        pathParams, queryParams, headerParams, formParams, postBody,
        authNames, contentTypes, accepts, returnType, null
      );
    }
     ...
     */
    logoutSession() {
      return this.logoutSessionWithHttpInfo()
        .then(function(response_and_data) {
          return response_and_data.data;
        });
    }
     ...
     * @return {Promise} a {@link https://www.promisejs.org/|Promise}, with an object containing data of type {@link Object} and HTTP response
     */
    openLoginPageWithHttpInfo(opts) {
      opts = opts || {};
      let postBody = null;
     ...
     */
    openLoginPage(opts) {
      return this.openLoginPageWithHttpInfo(opts)
        .then(function(response_and_data) {
          return response_and_data.data;
        });
    }
     ...
     * @param {module:model/OAuth2TokenGrantTypes} grantType Grant type. Determines the mechanism used to authorize the creation of the tokens.
     * @param {Object} opts Optional parameters
     * @param {String} opts.username Required for `password` grant type. The user’s username.
     * @param {String} opts.password Required for `password` grant type. The user’s password.
     * @param {String} opts.scope Scopes that the client wants to be included in the access token. List of space-delimited, case-sensitive strings
     * @param {String} opts.clientId The client identifier issued to the client during the registration process
     * @param {String} opts.clientSecret  The client secret. The client MAY omit the parameter if the client secret is an empty string.
     * @param {String} opts.code Required for `authorization_code` grant type. The value is what was returned from the authorization endpoint.
     * @param {String} opts.redirectUri Required for `authorization_code` grant type. Specifies the callback location where the authorization was sent. This value must match the `redirect_uri` used to generate the original authorization_code.
     * @param {String} opts.refreshToken Required for `refresh_token` grant type. The refresh token previously issued to the client.
     * @param {String} opts.state An opaque value used by the client to maintain state between the request and callback. The parameter SHOULD be used for preventing cross-site request forgery.
     * @param {Boolean} opts.setAsCookie If `true`, the access (and refresh) token will be set as cookie instead of the response body. (default to false)
